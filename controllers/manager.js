exports.install = function() {
	...
	// TODO
	F.route(CONFIG('manager-url') + '/api/todo/',               json_query,  ['*Todo']);
	F.route(CONFIG('manager-url') + '/api/todo/',               json_save,   ['post', '*Todo'], 500);
	F.route(CONFIG('manager-url') + '/api/todo/{id}/',          json_read,   ['*Todo']);
	F.route(CONFIG('manager-url') + '/api/todo/',               json_remove, ['delete', '*Todo']);
	F.route(CONFIG('manager-url') + '/api/todo/codelists/',     json_todo_codelists);
	F.route(CONFIG('manager-url') + '/api/todo/preview/',       json_todo_preview, ['post']);
	F.route(CONFIG('manager-url') + '/api/todo/stats/',         json_todo_stats, ['post']);
...
// Reads all post categories and manufacturers
function json_todo_codelists() {
	var self = this;
	self.json({ tags: F.global.todotags });
}

function json_todo_stats() {
	var self = this;
	NOSQL('todo').counter.count(self.body, self.callback());
}

function json_todo_preview() {
	var self = this;
	self.content(markdown(self.body.body || ''), 'text/html');
}
